...
NUM_THREADS = 1  # number of YOLO multiprocessing threads (set to 1 for Windows to fix BeeWare spawn issue)
...
HELP_MSG = """
    ...
    GitHub: https://github.com/zsniko/EDNet
    """
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            ...
            {DEFAULT_CFG_PATH} with the latest version
            """
        )
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from ednet.utils import ThreadingLocked
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        from ednet.utils.checks import check_version
        from ednet.utils.torch_utils import torch_distributed_zero_first
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not (correct_keys and correct_types and correct_version):
                LOGGER.warning(
                    "WARNING ⚠️ Settings reset to default values. This may be due to a possible problem "
                    f"with your settings or a recent package update. {self.help_msg}"
                )
                self.reset()

            if self.get("datasets_dir") == self.get("runs_dir"):
                LOGGER.warning(
                    f"WARNING ⚠️ Setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(f"Setting '{k}' must be of type '{t}', not '{type(v)}'. {self.help_msg}")
...
from ednet.utils.patches import imread, imshow, imwrite, torch_load, torch_save
