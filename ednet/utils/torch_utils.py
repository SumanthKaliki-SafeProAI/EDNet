...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    s = f"EDNet {__version__} ✅ Python-{PYTHON_VERSION} ✅ torch-{torch.__version__} ✅"
...
def model_info_for_loggers(trainer):
    ...
    if trainer.args.profile:  # profile ONNX and TensorRT times
        from ednet.utils.benchmarks import ProfileModels
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "") -> None:
    ...
    Example:
        ```python
        from pathlib import Path
        from ednet.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `ednet.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
        ...
        LOGGER.warning(f"WARNING ⚠️ Skipping {f}, not a valid model: {e}")
        return
...
def profile(input, ops, n=10, device=None):
    """
    Speed, memory and FLOPs profiler.

    Example:
        ```python
        from ednet.utils.torch_utils import profile
