...
from ednet.cfg import TASK2DATA, get_cfg, get_save_dir
from ednet.engine.results import Results

from ednet.nn.tasks import attempt_load_one_weight, guess_model_task, nn, yaml_model_load
from ednet.utils import (
    ARGV,
    ASSETS,
    DEFAULT_CFG_DICT,
    LOGGER,
...
class Model(nn.Module):
    ...
    validation, prediction, exporting, and benchmarking. It handles different types of models.
    ...
    Attributes:
        ...
        session (HUBTrainingSession): The HUB session, if applicable.
    ...
    def __init__(
        ...
        model sources, including local files, HUB models, and Triton Server models. The method
        ...
        Args:
            model (Union[str, Path]): Path or name of the model to load or create. Can be a local file path, a
                model name from HUB, or a Triton Server model.
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Returns:
            (List[ednet.engine.results.Results]): A list of prediction results, each encapsulated in a
                Results object.
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt", use_dill=True) -> None:
        ...
        updates = {
            ...
            "docs": "https://github.com/zsniko/EDNet",
        }
    ...
    def track(
        ...
        **kwargs,
        ...
        Returns:
            (List[ednet.engine.results.Results]): A list of tracking results, each a Results object.
        ...
        if not hasattr(self.predictor, "trackers"):
            from ednet.trackers import register_tracker
    ...
    def val(
        ...
        **kwargs,
        ...
        Args:
            validator (ednet.engine.validator.BaseValidator | None): An instance of a custom validator class for
                validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (ednet.utils.metrics.DetMetrics): Validation metrics obtained from the validation process.
    ...
    def benchmark(
        ...
        **kwargs,
        ...
        It uses the 'benchmark' function from the ednet.utils.benchmarks module. The benchmarking is
        ...
        """
        self._check_is_pytorch_model()
        from ednet.utils.benchmarks import benchmark
    ...
    def train(
        ...
        **kwargs,
        ...
        from a checkpoint, integrating with HUB, and updating model and configuration after training.

        When using HUB, if the session has a loaded model, the method prioritizes HUB training
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # HUB session with loaded model
            if any(kwargs):
    ...
    def tune(
        ...
        **kwargs,
        ...
        When Ray Tune is enabled, it leverages the 'run_ray_tune' function from the ednet.utils.tuner module.
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from ednet.utils.tuner import run_ray_tune
    ...
    def names(self) -> list:
        ...
        using the 'check_class_names' function from the ednet.nn.autobackend module. If the predictor is not
        ...
        Examples:
            ...
            ['person', 'bicycle', 'car', ...]
        """
        from ednet.nn.autobackend import check_class_names
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the callback system.
